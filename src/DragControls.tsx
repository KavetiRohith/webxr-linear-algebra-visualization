...
export const DragControls = () => {
    const { objects, selectedObjectId, updateObjectPosition, updateObjectRotation } = useLinePlaneStore();
    const { controllers } = useXR();
    const [isDragging, setIsDragging] = useState(false);
    const [dragType, setDragType] = useState<"move" | "rotate" | null>(null);
    const [startControllerPosition, setStartControllerPosition] = useState<Vector3 | null>(null);
    const [startObjectPosition, setStartObjectPosition] = useState<Vector3 | null>(null);
    ...
    useFrame(() => {
        ...
        if (!selectedObject || !startControllerPosition || !startObjectPosition) return;

        // Get controller position and calculate movement delta
        const controllerPosition = new Vector3().setFromMatrixPosition(controller.controller.matrixWorld);
        const delta = new Vector3().subVectors(controllerPosition, startControllerPosition);

        if (dragType === "move") {
            // Update object position
            updateObjectPosition(selectedObjectId, startObjectPosition.clone().add(delta));
    ...
    useEffect(() => {
        const handleSelectStart = (event: any) => {
            ...
            const controllerPosition = new Vector3().setFromMatrixPosition(controller.matrixWorld);
